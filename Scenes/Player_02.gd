...
func handle_movement(_delta):
	...
	velocity.x = (int(Input.is_action_pressed("right2")) - int(Input.is_action_pressed("left2"))) * speed
	if Input.is_action_pressed("right2"):
		...
		$AnimatedSprite.play("Courir")
	elif Input.is_action_pressed("dance2"):
...
func handle_jump():
	...
	if is_on_floor():
		...
		elif Input.is_action_pressed("right2") or Input.is_action_pressed("left2"):
			$AnimatedSprite.play("Courir")  
	else:
		$AnimatedSprite.play("Sauter") 
...
func handle_attack():
	if Input.is_action_just_pressed("push2") and $AnimatedSprite.animation != "Pousser":
		is_pushing = true
		velocity.x = 0
	...
	elif Input.is_action_just_released("push2"):
		$Attack_zone_01/CollisionShape2D.set_disabled(true)
		$Attack_zone_02/CollisionShape2D.set_disabled(true)
...
func _on_pushable_zone_02_area_entered(area):
	...
	if area.is_in_group("vide"):
		print(is_falling)
		var heart = get_parent().get_node("heart_2")
		if !is_falling:
			# Sauter vers le haut
...
func _on_Attack_zone_02_area_entered(area):
